...
public class InsnExecEventCreationTest extends ExecutionEventTestScafold {
  ...
  private void thenVerifyBasicEventPropertiesByPropertyNames(TraceEvent actualEvent) {
    ...
    assertEquals(vitalState[Profiler.THREAD_ID], Long.parseLong(actualEvent.getProp(InsnExecPropNames.THREAD_ID)) );
    assertEquals(vitalState[Profiler.TIMESTAMP], Long.parseLong(actualEvent.getProp(InsnExecPropNames.TIMESTAMP)) );
