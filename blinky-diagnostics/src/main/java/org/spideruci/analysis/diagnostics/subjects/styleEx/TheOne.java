 ...
 * traditional function composition provided by most programming languages. </br>
 * In this style of composing functions, we establish an abstraction ("the one") </br>
 * that serves as the glue between values and functions. This abstraction </br>
 * provides two main operations: a wrap operation that takes a simple value and </br>
 * returns an instance of the glue abstraction, and a bind operation that feeds a wrapped value to a function.
 * </p>
 */
public class TheOne{
	...
	public static Object top25_freqs(Object wf){
		...
		StringBuilder sb = new StringBuilder();
		for(int i=0;i<25;i++){
			sb.append(word_freqs_tmp.get(i).getKey()).append("  -  ").append(word_freqs_tmp.get(i).getValue()).append("\n");
		}
		return sb.toString();
	}
	...
	try {
				(new TFTheOne(args[0])).bind(TheOne.class.getMethod("read_file",Object.class)).
									...
									bind(TheOne.class.getMethod("top25_freqs",Object.class)).
									printme();
